...
**road, air, sea or drone** — what each option costs, and how long each takes, then sends an
agent to come and get it.
...
3. **Choose how it travels.** Four cards, priced live against this route and this parcel.
   ...
   200 km"). Try Nairobi → Westlands (road, drone) against Nairobi → Mombasa (road, air, sea).
...
band, because 40/km is a cross-town rate and absurd over 400 km. The other three are flat
per-km shared-capacity freight, which is why sea undercuts road over distance while taking a
day, and air costs multiples of both.
...
- **Duration** is time in motion plus each mode's handling at either end. Road prefers OSRM's
  measured driving time; a flight does not care how long the drive would have been.
...
**Icons for transport modes go through `<TransportGlyph>`.** Road, air and sea are Material
